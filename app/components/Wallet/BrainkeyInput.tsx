// TypeScript/functional-component port of the legacy BrainkeyInput.jsx
// (Phase 5, docs/UI_MIGRATION_PLAN.md). Mechanical, no logic changes.
// Security-sensitive per AGENTS.md: the raw brainkey phrase lives only
// in this component's own local state (as a controlled textarea, same
// as the original) and is forwarded via `onChange` - never logged
// (the existing `console.log` here only logs a dictionary *fetch*
// error, unrelated to the brainkey itself), never persisted.
//
// The module-level `dictionary_set` (populated once - synchronously from
// the bundled dictionary in Electron, or asynchronously via `fetch` in
// the browser build - and shared across every mounted instance) is kept
// exactly as-is, including the original's inefficiency: each new mounted
// instance re-fetches the dictionary from the server even if a previous
// instance already populated it (no "already loaded" check existed to
// port).
